...
def investigate_and_fix():
    ...
    with app.app_context():
        ...
        for parent_task in parent_tasks:
            ...
            child_tasks_new = Task.query.filter(
            ...
            completed_old = sum(
                1 for child in child_tasks_old if child.status == "complete"
            )

            # Check completion count using new method
            completed_new = sum(
                1 for child in child_tasks_new if child.status == "complete"
            )
            ...
            for i, child in enumerate(child_tasks_new, 1):
                ...
                print(
                    f"  {i}. {status_indicator} {child.description[:40]}... - {child.status}"
                )
...
def test_specific_tasks():
    ...
    with app.app_context():
        ...
        if james_task:
            ...
            print(
                f"Expected: {int((completed_count / len(child_tasks)) * 100) if child_tasks else 0}%"
            )
