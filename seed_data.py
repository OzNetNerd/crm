...
INDUSTRIES = [
    ...
    'Automotive', 'Energy', 'Telecommunications', 'Insurance', 'Logistics'
]
...
COMPANY_DATA = [
    ...
    {'name': 'DataStream Analytics', 'industry': 'Technology', 'website': 'https://datastream.com'},
    {'name': 'HealthPlus Systems', 'industry': 'Healthcare', 'website': 'https://healthplus.com'},
    {'name': 'AutoTech Innovations', 'industry': 'Automotive', 'website': 'https://autotech.com'},
    {'name': 'PowerGrid Solutions', 'industry': 'Energy', 'website': 'https://powergrid.com'},
    {'name': 'CommLink Networks', 'industry': 'Telecommunications', 'website': 'https://commlink.com'},
]
...
CONTACT_NAMES = [
    ...
    'Alex Cooper', 'Diana Foster', 'Tom Bradley', 'Sophia Turner', 'Nathan Hayes',
    'Olivia Parker', 'Lucas Reed', 'Emma Collins', 'Jake Morrison', 'Grace Bennett'
]
...
ROLES = [
    ...
    'Sales Manager', 'Technical Lead', 'Account Manager', 'Senior Developer', 'HR Director'
]
    ...
    for company in companies:
        ...
        for i in range(num_contacts):
            ...
            contact = Contact(
                ...
                phone=f"+1-{random.randint(100,999)}-{random.randint(100,999)}-{random.randint(1000,9999)}" if has_phone else None,
...
def create_opportunities(companies, contacts):
    ...
    for company in companies:
        ...
        for i in range(num_opps):
            ...
            opportunity = Opportunity(
                ...
                expected_close_date=date.today() + timedelta(days=random.randint(15, 365)),
    ...
    db.session.commit()
    print(f"Created {len(opportunities)} opportunities with values ranging ${min(opp.value for opp in opportunities):,} to ${max(opp.value for opp in opportunities):,}")
...
def create_comprehensive_tasks(companies, contacts, opportunities):
    ...
    for i in range(multi_entity_task_count):
        ...
        primary_entity_type, primary_entity_id, primary_entity_name = random.choice(entities)
        ...
        available_entities = [e for e in entities if not (e[0] == primary_entity_type and e[1] == primary_entity_id)]
        selected_entities = random.sample(available_entities, min(num_links, len(available_entities)))
        ...
        if parent_template in SUBTASK_TEMPLATES:
            ...
            for j, child_template in enumerate(selected_child_templates):
                ...
                if child_status == 'complete':
                    child_task.completed_at = datetime.now() - timedelta(days=random.randint(1, 20))
                ...
                inheritance_entities = random.sample(selected_entities, random.randint(1, len(selected_entities)))
    ...
    multi_entity_tasks = [t for t in tasks if t.task_type == 'single' and len(t.linked_entities) > 1]
    ...
    print(f"  - Task priority distribution: {[t.priority for t in tasks].count('high')} high, {[t.priority for t in tasks].count('medium')} medium, {[t.priority for t in tasks].count('low')} low")
...
def seed_database():
    ...
    print(f"💎 Deal Value Range: ${min(opportunity_values):,} to ${max(opportunity_values):,}")
    print(f"💵 Average Deal Size: ${sum(opportunity_values) // len(opportunity_values):,}")
    ...
    print("🚀 All entity types, filtering scenarios, and task workflows are ready for testing!")
