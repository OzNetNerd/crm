...
class BackgroundWorker:
    ...
    async def process_pending_meetings(self, db_session) -> int:
        ...
        pending_meetings = db_session.query(Meeting).filter(
            Meeting.analysis_status == "pending"
        ).limit(5).all()  # Process in small batches
        ...
        for meeting in pending_meetings:
            try:
                ...
                if result.get("status") == "success":
                    ...
                    logger.error(f"Failed to analyze meeting {meeting.id}: {result.get('error_message', 'Unknown error')}")
