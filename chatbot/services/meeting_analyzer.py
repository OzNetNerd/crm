...
from chatbot.models import Meeting, ExtractedInsight, Stakeholder, Task
...
class MeetingAnalyzer:
    ...
    async def analyze_meeting(
        self, meeting: Meeting, db_session: Session, force_reprocess: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            extraction_result = await client.extract_meeting_insights(
                transcript=meeting.transcript, meeting_title=meeting.title
            )
            ...
            if extraction_result.success:
                ...
                return {
                    ...
                    "insights_extracted": (
                        len(extraction_result.extracted_data.keys())
    ...
    async def _store_extraction_results(
        self, meeting: Meeting, extraction_result: ExtractionResult, db_session: Session
    ):
    ...
    async def _create_crm_entities(
        self, meeting: Meeting, extracted_data: Dict[str, Any], db_session: Session
    ) -> Dict[str, int]:
        """
        ...
        for attendee in attendees:
            if isinstance(attendee, dict) and attendee.get("name"):
                ...
                existing = (
                    ...
                    .filter(Stakeholder.name.ilike(f"%{attendee['name']}%"))
                    .first()
                )
    ...
    async def get_meeting_insights_summary(
        self, meeting_id: int, db_session: Session
    ) -> Dict[str, Any]:
        ...
        insights = (
            db_session.query(ExtractedInsight)
            .filter(ExtractedInsight.meeting_id == meeting_id)
            .all()
        )
        ...
        summary = {
            ...
            "analyzed_at": (
                meeting.analyzed_at.isoformat() if meeting.analyzed_at else None
            ),
    ...
    async def batch_analyze_meetings(
        self, meeting_ids: List[int], db_session: Session
    ) -> Dict[str, Any]:
