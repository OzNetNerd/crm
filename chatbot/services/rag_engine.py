...
class RAGEngine:
    ...
    def _analyze_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        patterns = {
            "company_info": ["company", "companies", "organization", "client", "business"],
            "contact_info": ["contact", "contacts", "person", "people", "who", "email", "phone"],
            "task_management": ["task", "tasks", "todo", "deadline", "due", "complete", "finish"],
            "meeting_info": ["meeting", "meetings", "discussed", "agenda", "attendees"],
            "opportunity_info": ["opportunity", "opportunities", "deal", "sales", "revenue", "pipeline"],
            "general_search": ["find", "search", "show", "list", "get", "what", "how many"],
            "analytics": ["report", "summary", "statistics", "count", "total", "analysis"]
        }
    ...
    def _assess_query_complexity(self, query: str, intent_scores: Dict[str, int]) -> str:
    ...
    async def _retrieve_context(
        ...
        try:
            ...
            if query_complexity == "simple":
                direct_results = await self._direct_database_search(query, primary_intent, db_session)
            ...
            if query_complexity in ["medium", "complex"]:
                ...
                context["method"] = "semantic_search" if query_complexity == "complex" else "hybrid"
            
            # Strategy 3: Related entity search
            entity_results = await self._related_entity_search(intent["entities"], db_session)
    ...
    async def _semantic_search(self, query: str, db_session: Session) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in similar_content:
                results.append({
                    ...
                    "content": item["text_content"][:200] + "..." if len(item["text_content"]) > 200 else item["text_content"],
    ...
    async def _related_entity_search(
        ...
        try:
            ...
            for name in entities.get("names", [])[:3]:  # Limit to avoid too many queries
                # Search companies
                companies = db_session.query(Company).filter(
    ...
    def _rank_and_filter_context(
        ...
        if context["sources"]:
            avg_relevance = sum(s.get("relevance_score", 0) for s in context["sources"]) / len(context["sources"])
            source_count_factor = min(len(context["sources"]) / 5.0, 1.0)  # More sources = higher confidence
