...
class QdrantService:
    ...
    def _connect(self):
        ...
        if self.client is None:
            try:
                ...
                logger.info(
                    f"Connected to Qdrant at {self.host}:{self.port} with optimized settings"
                )
        ...
        try:
            ...
            if not collection_exists:
                ...
                try:
                    ...
                    logger.info("Updated collection optimizer settings")
                except Exception as e:
                ...
                formatted_results.append(
                    {
                        "id": result.payload.get(
                            "doc_id", result.id
                        ),  # Use original doc_id if available
                        "score": result.score,
    ...
    def delete_entity_documents(self, entity_type: str, entity_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete entity documents {entity_type}:{entity_id}: {e}"
            )
            return False
