...
async def handle_streaming_response(response_data: dict, websocket: WebSocket, ws_manager):
    ...
    async for chunk in stream:
        if chunk["type"] == "chunk":
            ...
            await ws_manager.send_message(
                {"type": "streaming_chunk", "text": chunk["text"]},
                websocket
            )
    ...
    await ws_manager.send_message(
        {"type": "streaming_complete", "message": full_response},
        websocket
    )
...
async def handle_regular_response(response_text: str, websocket: WebSocket, ws_manager):
    ...
    await ws_manager.send_message(
        {"type": "bot_response", "message": response_text},
        websocket
    )