...
class EntityManager:
    ...
    def get_all_entities_for_selection():
        ...
        return {
            'companies': [{'id': c.id, 'name': c.name, 'type': 'company'} for c in companies],
            'contacts': [{'id': c.id, 'name': c.name, 'type': 'contact', 'company_name': c.company.name if c.company else None} for c in contacts],
            'opportunities': [{'id': o.id, 'name': o.name, 'type': 'opportunity', 'company_name': o.company.name if o.company else None} for o in opportunities]
        }
    ...
    def get_opportunity_stakeholders(opportunity_id, include_contacts=True):
        ...
        if include_contacts:
            for stakeholder in stakeholders:
                stakeholder['display_name'] = f"{stakeholder['name']} ({stakeholder['stakeholder_role']})"
...
class TeamManager:
    ...
    @staticmethod
    def assign_company_team_member(company_id, user_id, role, is_primary=False, access_level='read'):
    ...
    @staticmethod
    def assign_opportunity_team_member(opportunity_id, user_id, role, is_primary=False, access_level='read'):
...
class TaskEntityManager:
    ...
    def link_task_to_entities(task_id, entities):
        ...
        for entity in entities:
            entity_obj = EntityManager.get_entity_by_type_and_id(entity['type'], entity['id'])
            if entity_obj:
...
def assign_stakeholder_role(contact_id, opportunity_id, role, is_primary=False):
    ...
    return StakeholderManager.assign_stakeholder(contact_id, opportunity_id, role, is_primary)
