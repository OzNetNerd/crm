    ...
    if entity_type in ['all', 'contacts']:
        ...
        for contact in contacts:
            results.append({
                ...
                'type': 'contact',
                'title': contact.name,
                'subtitle': f'{contact.role} at {contact.company.name}' if contact.role else contact.company.name,
                'url': f'/contacts/{contact.id}'
            })
    
    if entity_type in ['all', 'opportunities']:
        opportunities = Opportunity.query.join(Company).filter(
            or_(
    ...
    if entity_type in ['all', 'tasks']:
        ...
        for task in tasks:
            results.append({
                ...
                'subtitle': f'Due: {task.due_date.strftime("%m/%d/%y")}' if task.due_date else 'No due date',
                'url': f'/tasks/{task.id}'
            })
