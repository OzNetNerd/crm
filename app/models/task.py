...
class Task(BaseModel):
    ...
    __include_properties__ = [
        "is_overdue", "opportunity_value", "company_name", "opportunity_name", "opportunity_stage",
        "stakeholder_opportunity_name", "stakeholder_opportunity_value", "task_type_badge",
        "can_start", "completion_percentage",
    ]
    ...
    description = db.Column(db.Text, nullable=False, info={"display_label": "Description", "required": True, "form_include": True})
    due_date = db.Column(db.Date, info={
        "display_label": "Due Date", "groupable": True, "sortable": True, "form_include": True,
        "date_groupings": {"overdue": "Overdue", "today": "Due Today", "this_week": "This Week", "later": "Later", "no_date": "No Due Date"},
    })
    priority = db.Column(db.String(10), default="medium", info={
        "display_label": "Priority", "groupable": True, "sortable": True, "form_include": True,
        "choices": {
    ...
    status = db.Column(db.String(20), default="todo", info={
        "display_label": "Status", "groupable": True, "sortable": True,
        "choices": {
            "todo": {"label": "To Do", "description": "Not started"},
            "in-progress": {"label": "In Progress", "description": "Currently working on"},
            "complete": {"label": "Complete", "description": "Finished"},
        },
    })
    next_step_type = db.Column(db.String(20), info={
        "display_label": "Next Step Type", "groupable": True, "sortable": True,
        "choices": {
            "call": {"label": "Call", "description": "Phone call"},
            "email": {"label": "Email", "description": "Send email"},
            "meeting": {"label": "Meeting", "description": "In-person or video meeting"},
    ...
    # Multi Task support
    task_type = db.Column(db.String(20), default="single", info={
        "display_label": "Task Type", "groupable": True, "sortable": True,
        "choices": {
    ...
    dependency_type = db.Column(db.String(20), default="parallel", info={
        "display_label": "Dependency Type", "groupable": True, "sortable": True,
        "choices": {"sequential": {"label": "Sequential", "description": "Must complete in order"}},
    })
    ...
    is_overdue = property(lambda self: bool(self.due_date and self.status != "complete" and self.due_date < datetime.utcnow().date()))
    linked_entities = property(lambda self: get_linked_entities(self.id))
    opportunity_value = property(lambda self: get_entity_attr(self.id, "opportunity", "value"))
    company_name = property(lambda self: get_company_name(self.id))
    opportunity_name = property(lambda self: get_entity_attr(self.id, "opportunity", "name"))
    opportunity_stage = property(lambda self: get_entity_attr(self.id, "opportunity", "stage"))
    task_type_badge = property(lambda self: {"parent": "Parent", "child": "Child"}.get(self.task_type, "Single"))
    ...
    def stakeholder_opportunity_name(self):
        ...
        return (self.opportunity_name or
                next((entity["entity"].opportunities[0].name for entity in get_linked_entities(self.id)
                      if entity["type"] == "contact" and entity["entity"] and entity["entity"].opportunities), None))

    @property
    def stakeholder_opportunity_value(self):
        """Get primary opportunity value for contact tasks"""
        return (self.opportunity_value or
                next((entity["entity"].opportunities[0].value for entity in get_linked_entities(self.id)
                      if entity["type"] == "contact" and entity["entity"] and entity["entity"].opportunities), None))
    ...
    def to_display_dict(self) -> Dict[str, Any]:
        ...
        display_fields = ["status", "priority", "next_step_type", "task_type", "dependency_type"]
